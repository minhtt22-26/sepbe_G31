...
export class HelperService {
  ...
  dateForward(date: Date, duration: { seconds: number }): Date {
  ...
  //Convert userID 1 less than userId 2
  getConversationUserIds(userA: number, userB: number) {
    return {
      user1Id: Math.min(userA, userB),
      user2Id: Math.max(userA, userB),
    }
  }
}
