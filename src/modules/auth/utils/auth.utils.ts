...
export class AuthUtil {
  ...
  constructor(
    ...
    this.jwtAccessTokenExpirationTimeInSeconds = this.configService.get<number>(
    ...
    this.jwtRefreshTokenExpirationTimeInSeconds =
      this.configService.get<number>('auth.jwt.refreshToken.expiresIn')!
    ...
    this.forgotPasswordTokenLength = this.configService.get<number>(
      'auth.forgotPassword.tokenLength',
    )!
    this.forgotPasswordExpiredInSecond = this.configService.get<number>(
      'auth.forgotPassword.expiredInMinutes',
    )!
    this.forgotPasswordResendInSecond = this.configService.get<number>(
      'auth.forgotPassword.resendInMinutes',
    )!
    this.forgotPasswordBaseUrl = this.configService.get<string>(
      'auth.forgotPassword.baseUrl',
    )!
  ...
  createAccessTokens(jti: string, payload: IAuthAccessTokenPayload): string {
  ...
  createForgotPassword(): IForgotPasswordCreate {
    ...
    const expiredAt = this.helperService.dateForward(
      today,
      this.helperService.dateCreateDuration({
        seconds: this.forgotPasswordExpiredInSecond,
      }),
    )
