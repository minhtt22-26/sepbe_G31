...
export class SessionService {
  ...
  async getList(userId: number): Promise<SessionListResponseDto[]> {
    const userIdNum = this.sessionUtil.toNumberId(userId)
  ...
  async revoke(
    ...
    await this.sessionRepository.revoke(userIdNum, sessionId)
  }

  async revokeAll(userId: number) {
    const userIdNum = this.sessionUtil.toNumberId(userId)
    await this.sessionRepository.revokeAll(userIdNum)
  }
}
