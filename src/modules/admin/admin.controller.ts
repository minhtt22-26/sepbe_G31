...
import { AuthJwtAccessProtected, AuthRoleProtected } from '../auth/decorators/auth.jwt.decorator';
...
export class AdminController {
  ...
  async getStatistics(@Query('year') year?: string) {
    return this.adminService.getStatistics(year ? parseInt(year, 10) : undefined);
  }
}
