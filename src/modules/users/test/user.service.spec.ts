...
describe('UserService', () => {
  ...
  describe('signUp', () => {
    ...
    it('Normal: should successfully sign up a user when email and username are unique', async () => {
      ...
      // Verify
      expect(userRepository.findUserWithByEmail).toHaveBeenCalledWith(
        signUpDto.email,
      )
      expect(userRepository.findUserWithByUserName).toHaveBeenCalledWith(
        signUpDto.userName,
      )
    ...
    it('Abnormal: should throw BadRequestException when email already exists', async () => {
      // Setup mocks
      mockUserRepository.findUserWithByEmail.mockResolvedValue({
        id: 1,
    ...
    it('Boundary: should throw BadRequestException when username already exists', async () => {
      // Setup mocks
      mockUserRepository.findUserWithByEmail.mockResolvedValue(null)
      mockUserRepository.findUserWithByUserName.mockResolvedValue({
        id: 1,
  ...
  describe('loginCrendential', () => {
    ...
    it('Normal: should successfully login with valid credentials', async () => {
      ...
      ;(mockHelperService.dateForward).mockReturnValue(new Date())

      // Execute
      const result = await service.loginCrendential(loginDto)

      // Verify
      expect(result).toEqual({ tokens: mockTokens })
      expect(userRepository.resetPasswordAttempt).toHaveBeenCalledWith(
        mockUser.id,
      )
    ...
    it('Abnormal: should throw NotFoundException when user does not exist', async () => {
      ...
      // Execute & Verify
      await expect(service.loginCrendential(loginDto)).rejects.toThrow(
        NotFoundException,
      )
    ...
    it('Boundary: should throw ForbiddenException when password attempts exceed limit', async () => {
      ...
      await expect(service.loginCrendential(loginDto)).rejects.toThrow(
        'Bạn đã nhập sai mật khẩu quá nhiều lần, vui lòng thử lại sau',
      )
  ...
  describe('Worker Profile methods', () => {
    ...
    describe('createWorkerProfile', () => {
      ...
      it('Abnormal: should throw BadRequestException when profile already exists', async () => {
        ...
        await expect(
          service.createWorkerProfile(userId, profileDto),
        ).rejects.toThrow(BadRequestException)
        await expect(
          service.createWorkerProfile(userId, profileDto),
        ).rejects.toThrow('Hồ sơ của bạn đã tồn tại')
      })
    ...
    describe('getWorkerProfile', () => {
      ...
      it('Abnormal: should throw NotFoundException when profile does not exist', async () => {
        ...
        await expect(service.getWorkerProfile(userId)).rejects.toThrow(
          NotFoundException,
        )
      })

      it('Boundary: should throw Error when repository.getWorkerProfile fails', async () => {
        mockUserRepository.getWorkerProfile.mockRejectedValue(
          new Error('DB Error'),
        )
        await expect(service.getWorkerProfile(userId)).rejects.toThrow(
          'DB Error',
  ...
  describe('Authentication & Tokens', () => {
    ...
    describe('refreshToken', () => {
      ...
      it('Normal: should refresh tokens successfully', async () => {
        ...
        expect(userRepository.updateLastActivity).toHaveBeenCalledWith(
          mockUser.id,
        )
      })
      ...
      it('Abnormal: should throw UnauthorizedException when session not found', async () => {
        ...
        await expect(
          service.refreshToken(mockUser, refreshToken),
    ...
    describe('logout', () => {
      it('Normal: should call sessionService.revoke', async () => {
        ...
        expect(sessionService.revoke).toHaveBeenCalledWith(1, 's1')
      })
      ...
      it('Abnormal: should handle session already revoked or not found', async () => {
        ...
        await expect(service.logout(1, 's1')).rejects.toThrow(NotFoundException)
      })
      ...
      it('Boundary: should throw Error when repository error occurs', async () => {
        ...
        await expect(service.logout(1, 's1')).rejects.toThrow('Revoke Error')
      })
    })
    ...
    describe('loginWithSocial', () => {
      ...
      it('Boundary: should throw Error when repository creation fails', async () => {
        mockUserRepository.findUserWithByEmail.mockResolvedValue(null)
        mockUserRepository.createBySocial.mockRejectedValue(
  ...
  describe('Password Management', () => {
    ...
    describe('forgotPassword', () => {
      it('Normal: should create token and send email successfully', async () => {
        ...
        mockUserRepository.findLatestForgotPasswordRequest.mockResolvedValue(
          null,
        )
        mockAuthUtil.createForgotPassword.mockReturnValue({
          token: 't',
          expiredAt: new Date(),
          link: 'l',
        })
      ...
      it('Boundary: should throw BadRequestException when rate limited', async () => {
        ...
        ;(mockHelperService.dateCreate).mockReturnValue(new Date())
        ;(mockHelperService.dateForward).mockReturnValue(
    ...
    describe('resetPassword', () => {
      it('Normal: should update password and mark token used', async () => {
        ...
        mockUserRepository.findValidForgotPasswordToken.mockResolvedValue(
          tokenRecord,
        )
        ...
        expect(userRepository.markForgotPasswordTokenUsed).toHaveBeenCalledWith(
          tokenRecord.id,
        )
      })
      ...
      it('Abnormal: should throw BadRequestException when token invalid', async () => {
        ...
        await expect(
          service.resetPassword({ token: 't', newPassword: 'p' }),
        ).rejects.toThrow(BadRequestException)
      })
